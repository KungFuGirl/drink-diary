...
# add all australian states and wine regions
australian_wine_regions_data.each do |state, regions|
  aus_state = State.find_or_create_by!({ name: state, country: australia })
  regions.each do |region|
    WineRegion.find_or_create_by!({ name: region, country: australia, state: aus_state })
  end
end
...
beechworth_appellation = Appellation.find_or_create_by!( name: 'Beechworth', wine_region: WineRegion.find_by(name: "NE Victoria"))
